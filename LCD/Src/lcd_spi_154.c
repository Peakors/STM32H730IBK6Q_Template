/***
   ...
   *  @description
	*
	*	实验平台：反客STM32H730IBK6Q核心板 （型号：FK730M1-IBK6Q）+ 1.54寸液晶模块（型号：SPI154M1-240*240）
...
void HAL_SPI_MspInit(SPI_HandleTypeDef *hspi) {
    ...
    if (hspi->Instance == SPI5) {
        ...
        __HAL_RCC_GPIOF_CLK_ENABLE();    // 使能 SPI5 GPIO 时钟
        __HAL_RCC_GPIOH_CLK_ENABLE();    // 使能 SPI5 GPIO 时钟
...
// 初始化 SCK、MOSI
        GPIO_InitStruct.Pin = GPIO_PIN_6 | GPIO_PIN_5;      // SCK、MOSI引脚
...
// 初始化片选引脚，使用硬件 SPI 片选
        GPIO_InitStruct.Pin = GPIO_PIN_11;                 // 片选 引脚
        GPIO_InitStruct.Alternate = GPIO_AF5_SPI5;               // 复用到SPI5，复用线5
...
// 在 main.c文件 SystemClock_Config()函数里设置 SPI5 的内核时钟为137.5M，然后再经过2分频得到 68.75M 的SCK驱动时钟
...
void MY_SPI_CloseTransfer(SPI_HandleTypeDef *hspi) {
    ...
    __HAL_SPI_DISABLE_IT(hspi,
                         (SPI_IT_EOT | SPI_IT_TXP | SPI_IT_RXP | SPI_IT_DXP | SPI_IT_UDR | SPI_IT_OVR | SPI_IT_FRE |
                          SPI_IT_MODF));
